            ...
            for x in aux_structures[z]:
                ...
                if not (x[0] in paths.keys()):
                    ...
                    if n!=0:
                        ...
                        if not (x[0] in paths):
                            the_sum=(paths[z][1]+x[3])    
                            temp=(paths[z][0][:],the_sum)
                            paths.update({x[0]:temp})
                            if not (z in paths[x[0]][0]):
                                paths[x[0]][0].append(z)
                        else:
                            if (paths[z][1]+x[3])<paths[x[0]][1]:
                                oth_sum=(paths[z][1]+x[3])
                                oth_temp=(paths[z][0][:],oth_sum)
                                paths.update({x[0]:oth_temp})
                                if not (z in paths[x[0]][0]):
                                    paths[x[0]][0].append(z)
...
if __name__ == '__main__':
    ...
    #implementing server:
    #from_spot = (42.3765 , -71.2356)
    #to_spot = (42.5195, -70.8967)
    #aux = build_auxiliary_structures('resources/cambridge.nodes', 'resources/cambridge.ways')
    #print(to_local_kml_url(find_fast_path(aux, from_spot, to_spot)))
    pass
